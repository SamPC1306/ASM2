...
router.post('/login', async function(req, res, next) {
  let user_name = req.body.username; // HTML Login
  let pass_word = req.body.password; // HTML Login
...
router.get('/logout',(req,res) => {
  req.session.destroy();  // logout những thứ đã lưu vào session
  res.redirect('/');
});
...
router.get('/admin', async function(req, res, next) {
  ...
  if (req.session.username)
  {
    ...
    let table_string = await table_crud('users', role); // hiện bảng
...
router.get('/shop_info', async function(req, res, next) {
  ...
  if (req.session.username)
  {
    ...
    table_string = await table_crud('products', role, department_id); // hiện bảng
...
/* POST crud for ADMIN AND SHOP page */
...
await admin_crud(req_body, table_name);
  ...
  // quăng lại mình sau khi thực hiện hành động
  if (role === 'admin') {
    res.redirect('/users/admin');
  } else {
    res.redirect('/users/shop_info');
...
router.get('/director', async function(req, res, next) {
  ...
  // nếu người dùng chưa đăng nhập thì
  if (user_name) // check nếu không rỗng
  {
